...
import { fetchCatalog, fetchCatalogAgent, fetchRules } from '@/features/pipeline-editor/scripts/pipelineEditorApi'
...
describe('fetchCatalogAgent', () => {
  it('sends the given projectId as the `project` query param', async () => {
    const fetchMock = stubApi()
    await fetchCatalogAgent('agent-x', 'P1')
    const url = String(fetchMock.mock.calls[0][0])
    expect(url).toContain('/api/catalog-agent')
    expect(url).toContain('id=agent-x')
    expect(url).toContain('project=P1')
  })

  it('omits the `project` param when no projectId is given (back-compat)', async () => {
    const fetchMock = stubApi()
    await fetchCatalogAgent('agent-x')
    const url = String(fetchMock.mock.calls[0][0])
    expect(url).toContain('/api/catalog-agent')
    expect(url).toContain('id=agent-x')
    expect(url).not.toContain('project=')
