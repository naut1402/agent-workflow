...
flowchart TB
  ...
  subgraph core["src/core — ổn định"]
    ...
    contracts["contracts — fs, sanitize, appSettings"]
    lib["lib — *Utils / *Lib / phase"]
    log["log — driver + emit"]
    registry["registry"]
  end
...
3. Không sửa `apiServer` registry tay — để glob/`registerFeatureRoutes` nạp.
4. Schema domain để trong feature; đừng đẩy vào `core/contracts` trừ shell thật sự.
5. Dùng `*Utils` / `*Lib` có sẵn; không copy slug/YAML.
6. Chạy `bun run build` nếu module đụng cả FE lẫn Node (tránh `node:fs` lọt bundle).
