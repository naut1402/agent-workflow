...
Mọi git/commit/push thực hiện trong worktree đó, theo [`git-convention.md`](git-convention.md). Không `cd` về cây chính để sửa file task khác.
...
Đừng `git push` lại branch vừa merge (origin có thể đã xóa → tạo branch rác, xem git-convention).
