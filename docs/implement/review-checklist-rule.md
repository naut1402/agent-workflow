...
- **Kiểm tra kỹ khi đổi chữ ký `fileHelper`**: đã tính tới `vue-tsc` (vd `readDir` + `withFileTypes`, `watch`) và chạy typecheck; call site khớp API wrapper (không truyền arg thừa như `'utf8'` nếu helper đã cố định encoding).
