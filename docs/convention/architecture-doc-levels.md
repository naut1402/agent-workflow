...
- Đây là cấp **duy nhất được phép** nêu tên file, hàm, bảng schema, đường dẫn cụ thể — đúng bản chất "code-level detail". Bất biến kiến trúc (tên hàm/file sanitize cụ thể) đã thành checklist review ở `AGENTS.md` §4, không đặt ở cấp này.
...
Tài liệu áp dụng quy ước này: [`docs/architecture/`](../architecture/README.md). Quy ước trình bày markdown dùng chung: [`writing-guideline.md`](../agent-rules/writing-guideline.md) §1–§4.
