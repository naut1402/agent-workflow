...
Đọc "Rule viết tài liệu" (doc-writing) từ chỉ dẫn agent của project: `CLAUDE.md` / `AGENTS.md` ở root repo, rồi file rule doc-writing mà hub đó trỏ tới — rule project ưu tiên hơn khi xung đột, format `investigate.md` **bắt buộc** theo rule đó. Không nguồn nào đọc được thì dùng skeleton mặc định của skill `survey-codebase` làm fallback và ghi rõ trong báo cáo là đã chạy ở nhánh fallback cuối. **Không dừng pipeline** chỉ vì thiếu rule project.
