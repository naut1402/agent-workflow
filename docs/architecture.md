...
| Knowledge | `src/features/knowledge/business/` | File driver đa root (`scope → base`: `project`/`system` ở data root, `global` ở `registryHome()`) cho **entry**; **collection + metadata tag + alias tag** ở `dashboard.sqlite` (`knowledgeDb.ts` · `collections.ts` · `tags.ts`, phân vùng theo cột `store_key` = đường dẫn store base). Gán tag vẫn nằm ở front-matter entry. `collectionsYaml.ts` chỉ còn phục vụ lệnh migrate. |
...
**Tầng DB `src/backend/db/`.** Một file SQLite dùng chung cho mọi subsystem chuyển khỏi lưu trữ file-based. Hai subsystem đang dùng: log backend `sqlite` (opt-in) và collection/tag của knowledge (luôn bật).
...
- **Fetch URL người dùng** phải qua `fetchUrlSafe` (https-only, chặn private host) — tránh SSRF. **Ngoại lệ duy nhất: endpoint MCP** (`http`/`sse`) đi qua `assertMcpEndpoint` của feature `mcp` (§4.2): `https` mọi host · `http` **chỉ** loopback/private. Ngoại lệ này tồn tại vì ca dùng chính của MCP là server chạy local (playwright, serena), mà `fetchUrlSafe` chặn đúng nhóm đó; nới `fetchUrlSafe` sẽ mở bề mặt SSRF ở mọi call site khác. Chốt riêng, 🚫 không dùng làm tiền lệ cho endpoint mới.
