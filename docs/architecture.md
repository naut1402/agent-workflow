...
> Tài liệu bám **cấu trúc thật** của nhánh hiện tại. Ngoại lệ đuôi file: `shared/agentMarkdown.js` và `server/runner-cli.mjs` cố ý **chưa** chuyển `.ts` — ghi đúng đuôi.
...
Nền tảng FE / shell: `composables/*` (`useAsyncState`, `useDrop`, `useLocalToggle`, `useSearch`, `useSortable`, …), `lib/workflowSteps.ts`, `markdown.ts`, `ui/RailIcon.vue` (+ primitives `C*`), `i18n/`, `shell/keys.ts` (typed `InjectionKey` cho `navigateToMode` / `reloadProjects`).

Khác với `shared/` ở repo root (Zod schema + helper isomorphic/Node dùng chung FE↔BE — xem §2.5).
...
├── vite.config.ts          # Vite + Vue plugin + devTeamApi middleware; dev port 5174
├── package.json            # ESM ("type":"module"); bun; scripts: dev/build/preview/serve/start/mcp/
│                           #   typecheck/lint/test/test:fe/test:e2e/test:all
...
├── eslint.config.js        # ESLint flat config (giữ .js — không cần TS loader)
...
├── tests/                  # unit tests mirror cây source (bun test: server/mcp · vitest: src/core + shared)
├── test-e2e/               # @playwright/test specs + fixtures/.dev-team-agent/
├── docs/                   # Tài liệu: architecture.md (file này); i18n.md; ui-buttons.md; scss-adoption.md — không chứa evidence test / tài liệu theo task
