...
- **`migrateLogs.ts`** + `scripts/migrate-logs-to-sqlite.ts` — nạp JSONL cũ vào bảng `log_entries`. Chỉ đọc, **không xoá** file nguồn; **không idempotent** (chạy lại sinh bản ghi trùng).
