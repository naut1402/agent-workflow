...
> [!IMPORTANT]
> <span style="color:#a371f7">Tài liệu tồn tại để người đọc **quét nhanh và ra quyết định**, không phải để chứa đủ thông tin. Mọi nguyên tắc, quy tắc và quy ước dưới đây phục vụ hai việc đó.</span>
...
| Màu | Callout | Loại | Vi phạm thì sao | Mã màu |
|---|---|---|---|---|
| 🔴 Đỏ | `[!CAUTION]` | **Quy tắc** — đúng/sai máy móc, thường có code đang parse | **Hỏng thật**: công cụ đọc sai, viewer cắt đôi khối code, control không render | `#e5534b` |
| 🟡 Vàng | `[!WARNING]` | **Nguyên tắc** — ràng buộc nền suy ra từ triết lý, cần đọc hiểu mới áp được | Sản phẩm sai cấu trúc; người duyệt không tìm được thứ cần đọc | `#d29922` |
| 🟣 Tím | `[!IMPORTANT]` | **Triết lý** — lý do tồn tại của cả tài liệu | Làm đúng hết mọi rule mà vẫn ra sản phẩm vô dụng: sai mục tiêu ngay từ đầu | `#a371f7` |
| 🔵 Xanh dương | `[!NOTE]` | **Phương châm** — chọn thế nào khi hai cái cùng đúng, hoặc khi xung đột | Không hỏng ngay, nhưng đi sai hướng ở mỗi ngã ba | `#4493f8` |
| 🟢 Xanh lá | `[!TIP]` | **Quy ước** — thống nhất cho dễ đọc | Không sai, chỉ khó đọc và thiếu nhất quán | `#3fb950` |
...
### 5.1 Fallback cho renderer không hỗ trợ callout

> [!CAUTION]
> <span style="color:#e5534b">Mọi callout phải bọc nội dung trong `<span style>` mang mã màu tương ứng ở bảng trên — dashboard render bằng `marked` (`src/frontend/lib/markdownLib.ts`), không hiểu cú pháp callout nên chỉ còn màu của `<span>`.</span>
...
- **Callout chứa list thì bọc từng gạch đầu dòng** — `<span>` là thẻ inline, không ôm được block.
- **Không thêm nhãn chữ** kiểu `QUY TẮC —`: khi callout render được thì nhãn thành thừa.
