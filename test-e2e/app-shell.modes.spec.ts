...
// E2E capture for the app-shell: sidebar mode switching across modes (the shell
// + useTaskPolling extraction). Confirms each mode's root panel mounts and the
// runner mode (RunnerConfigPanel, moved to features/runner) works.
...
test('sidebar switches across modes incl. runner (capture)', async ({ page }, testInfo) => {
  ...
  await capture(page, testInfo, 'runner-config')
