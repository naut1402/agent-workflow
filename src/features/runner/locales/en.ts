...
export default {
  ...
  messages: {
    ...
    confirmDeleteCredential: 'Delete credential {label}?',
  ...
  connectionDialog: {
    ...
    providerHint: 'Pick a configured provider (interface + base URL) — select or create a credential below',
    ...
    modelField: 'Model',
    modelHint: 'Model used by this connection — type to filter or enter any name, pick one or leave empty',
    ...
    extraToolsLabel: 'Extra tools',
    extraToolsHint: 'Opt-in tools beyond read/write/edit/list files — off by default, applies only to this connection',
    extraToolShell: 'Shell',
    extraToolShellHint: 'Lets the model run allowlisted binaries (bun/npm/npx/pnpm/yarn/node/tsc) in the workspace',
    extraToolGit: 'Git',
    extraToolGitHint: 'Read-only git status/diff/log of the workspace',
    extraToolSearch: 'Search',
    ...
    secretValueEditHint: 'Leave empty to keep the current secret — fill in only to replace it',
  ...
  providerDialog: {
    ...
    intro: 'Provider setup (interface + base URL) is shared across connections — credential is chosen per connection.',
