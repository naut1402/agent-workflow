...
export default {
  ...
  messages: {
    ...
    confirmDeleteCredential: 'Xóa credential {label}?',
  ...
  connectionDialog: {
    ...
    providerHint: 'Chọn provider đã cấu hình (interface + base URL) — credential chọn hoặc tạo ở bên dưới',
    ...
    extraToolsLabel: 'Tool mở rộng',
    extraToolsHint: 'Tool opt-in ngoài read/write/edit/list file — mặc định tắt, chỉ áp dụng cho connection này',
    extraToolShell: 'Shell',
    extraToolShellHint: 'Cho phép model chạy các binary trong allowlist (bun/npm/npx/pnpm/yarn/node/tsc) trong workspace',
    extraToolGit: 'Git',
    extraToolGitHint: 'Xem git status/diff/log của workspace (chỉ đọc)',
    ...
    secretValueEditHint: 'Để trống nếu không đổi secret hiện tại — chỉ điền khi muốn thay bằng secret mới',
  ...
  providerDialog: {
    ...
    intro: 'Provider (interface + base URL) dùng chung cho nhiều connection — credential chọn riêng ở từng connection.',
