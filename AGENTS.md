...
Nguồn quy ước **hub** cho mọi AI agent trong repo này.

- **Đọc file này trước**, rồi mở rule tương ứng với bước đang làm (bảng §3).
- **Rule chi tiết nằm ở [`.claude/rules/`](.claude/rules/)** — mỗi file một category, dashboard quét qua `GET /api/rules` và gắn cho từng bước pipeline.
- **Tài liệu mô tả hệ thống nằm ở [`docs/`](docs/)** — kiến trúc, domain event, i18n, quy ước UI. Nếu xung đột về **bất biến** hoặc coupling tối thiểu, coi file này là đúng.
...
- **Config** (pipeline, custom agent, template, knowledge) và **artifact markdown** — đọc/ghi được (ghi qua `PUT /api/artifact`).
...
├── test-e2e/
├── docs/         # architecture, event-catalog, i18n, ui-buttons, diagram/, template/
└── .claude/
    └── rules/    # rule cho công cụ AI, theo category
```
...
| Bước | Category | Rule |
|---|---|---|
| Investigate · Design | `doc-writing` | [`doc-writing.md`](.claude/rules/doc-writing.md) — bố cục `investigate.md` / `design.md`, quy tắc trình bày |
| Implement | `coding` | [`coding-guideline.md`](.claude/rules/coding-guideline.md) · [`feature-architecture-guideline.md`](.claude/rules/feature-architecture-guideline.md) · [`mode-registry-guideline.md`](.claude/rules/mode-registry-guideline.md) |
| Review | `coding` + `test` | [`review-checklist-guideline.md`](.claude/rules/review-checklist-guideline.md) · [`testing.md`](.claude/rules/testing.md) |
| PR | `git-pr` | [`git-pr.md`](.claude/rules/git-pr.md) · [`git-worktree.md`](.claude/rules/git-worktree.md) · [`pr-todo-debt.md`](.claude/rules/pr-todo-debt.md) |
...
| Task | Đọc thêm |
|------|----------|
| Viết/sửa code feature | [`feature-architecture-guideline.md`](.claude/rules/feature-architecture-guideline.md) + [`coding-guideline.md`](.claude/rules/coding-guideline.md) + bất biến §4 |
| Thêm mode mới ở FE shell (`App.vue`) | [`mode-registry-guideline.md`](.claude/rules/mode-registry-guideline.md) — checklist §5 |
| Review PR | [`review-checklist-guideline.md`](.claude/rules/review-checklist-guideline.md) — mục **Dữ liệu & An toàn** có domain event khi đụng persist |
| Test / CI | [`testing.md`](.claude/rules/testing.md) |
| Commit / PR / docs | [`git-pr.md`](.claude/rules/git-pr.md) — tách commit §6 khi PR nhiều xử lý |
| Hoãn docs/test (hotfix, POC) | [`pr-todo-debt.md`](.claude/rules/pr-todo-debt.md) — gate CI chỉ khi PR `dev/x.y.z/main` → `main` |
| Agent chạy song song | [`git-worktree.md`](.claude/rules/git-worktree.md) |
| Viết `investigate.md` / `design.md` | [`doc-writing.md`](.claude/rules/doc-writing.md) |
...
- [ ] **Đối chiếu catalog với code khi implement/review** — xem [`review-checklist-guideline.md`](.claude/rules/review-checklist-guideline.md) mục **Dữ liệu & An toàn**.

### Implement — test suite

Áp dụng cho **mọi** thay đổi code:

- [ ] **Tra danh mục suite** [`testing.md`](.claude/rules/testing.md) §4 — xác định suite nào phủ vùng vừa sửa (có thể nhiều suite, khác runner).
...
- [ ] **Cập nhật rule** trong [`.claude/rules/`](.claude/rules/) ngay trong cùng thay đổi — rule lệch code là nợ, không phải chi tiết.
